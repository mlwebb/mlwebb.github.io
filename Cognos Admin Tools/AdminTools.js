...
function createAndAppendStyles() {
  ...
  window.showToast = function (
    ...
    if (!validTypes.includes(options.type)) {
      console.warn(`Invalid toast type: ${options.type}. Defaulting to 'info'.`);
      options.type = "info";
    }
  ...
  window.updateProgressToast = function (id, increment = 1, newMessage = null) {
    ...
    var percentage = Math.min((toast.currentCount / toast.totalCount) * 100, 100);
  ...
  function fakeProgressBar(maxValue, message, title, steps = 20, interval = 1000) {
    
  
    let toast = showProgressToast(maxValue, message, title);
    let currentProgress = 0;
    let increment = maxValue / steps;
    setTimeout(showToast("message","Error: ",5000,{type:"error"}), 5000)
    setTimeout(showToast("message","Warning: ",5000,{type:"warning"}), 5000)
    setTimeout(showToast("message1","Info: ",5000,{type:"info"}), 5000)
    setTimeout(showToast("message2","Info: ",5000,{type:"info"}), 5000)
    setTimeout(showToast("message2","Info: ",5000,{type:"info"}), 5000)
    setTimeout(showToast("message","Success: ",5000,{type:"success"}), 5000)
  
    function updateProgress() {
      currentProgress += increment;
      let roundedProgress = Math.min(Math.round(currentProgress), maxValue);
      let progressMessage = `${roundedProgress}%`;
  
      if (roundedProgress >= maxValue) {
        progressMessage += " Completed";
      }
  
      updateProgressToast(toast, increment, progressMessage);
      setTimeout(showToast("message1","Info: ",5000,{type:"info"}), 5000)
      setTimeout(showToast("message2","Info: ",5000,{type:"info"}), 5000)
      setTimeout(showToast("message2","Info: ",5000,{type:"info"}), 5000)
      setTimeout(showToast("message","Success: ",5000,{type:"success"}), 5000)
  ...
  async function fetchFolderObjects(id, objectType) {
    try {
      const url =
        __glassAppController.glassContext.gateway +
        `/v1/expressbus/content-manager?searchPath=storeID('${id}')//${objectType}&properties=id,defaultName,searchPath`;
  ...
  async function fetchFolderObjectTypes(id) {
    try {
      const url =
        __glassAppController.glassContext.gateway +
        `/v1/expressbus/content-manager?searchPath=storeID('${id}')//*&properties=type`;
      ...
      let csvcontent = convertArrayToCSV(uniqueTypes)
      downloadCSV(csvcontent, `Cognos Object Types - ${formattedDate}.csv`)
  ...
  const objectTypes = {
    ...
    folder: {
      ...
      actions: {
        action1: {
          name: "Export All Object Types in Folder (CSV)",
          execute: async (objectDetails) => {
            await fetchFolderObjectTypes(objectDetails.id);
          },
        },
        action2: {
          name: "Export List of Reports (CSV)",
          execute: async (objectDetails) => {
            await generateAndDownloadReportListCSV(objectDetails);
          },
        },
        action3: {
          name: "Export All Filters for All Reports (CSV)",
          execute: async (objectDetails) => {
  ...
  async function generateAndDownloadReportListCSV(objectDetails) {
    try {
      const reportArray = await fetchFolderObjects(objectDetails.id, "report");
      ...
      downloadCSV(csvContent, "List of Reports Report - " + formattedDate);
  ...
  const referenceElement = document.getElementById("com.ibm.bi.content.searchBarPluginContainer");
  
  // Create a button element for the gear icon
  const gearButton = document.createElement("button");
  gearButton.className = "gear-icon";
  gearButton.setAttribute("onclick", "showDialogAndGetObjectId()");
  
  // Create an img element with the provided base64 SVG
  const imgElement = document.createElement("img");
  imgElement.src = "data:image/svg+xml;base64,";
  imgElement.alt = "Settings";
  imgElement.width = 24;
  imgElement.height = 24;
  
  // Append the img to the button
  gearButton.appendChild(imgElement);
  
  // Insert the button before the reference element
  referenceElement.parentNode.insertBefore(gearButton, referenceElement);
